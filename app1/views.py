...
db = pymysql.connect(host='localhost',user= 'root',password= '', database='wishlist')
c = db.cursor()
...
def Customer_Registration(request):
    ...
    if request.POST:
        ...
        type = "Customer"
        qry = f"select count(*) from login where username='"+email+"'"
        c.execute(qry)
        d = c.fetchone()
        if d[0] > 0:
            msg = "User already registered"
        else:
            qry = "insert into cusreg(cname,address,pincode,gender,age,district,location,email,mobile,password) values('" + cname + "','" + address + "','" + \
                pincode + "','" + gender + "','" + age + "','" + district + "','" + \
                location + "','" + email + "','" + mobile + "','" + password + "')"
            qr = "insert into login values('" + email + \
                "','" + password + "','" + type + "')"
            c.execute(qry)
            c.execute(qr)
            db.commit()
...
def SignIn(request):
    ...
    if request.POST:
        ...
        password = request.POST.get("password")
        try:
            m = "select * from login where username='" + \
            ...
            if ds[2] == 'Admin':
                ...
                c.execute("select * from cusreg where email='" +
                          email+"' and password='"+password+"'")
                ds = c.fetchone()
                ...
                c.execute("select * from shopreg where email='" +
                          email+"' and password='"+password+"'")
...
def Club_Registration(request):
    if request.POST:
        ...
        status = "Registered"
        type = "shop"
        qry = "insert into shopreg(clname,district,city,phone,email,reg_no,password,status) values('" + cname + "','" + district + \
            "','" + location + "','" + mobile + "','" + email + \
            "','" + regno + "','" + password + "','" + status + "')"
        qr = "insert into login values('" + email + \
            "','" + password + "','" + type + "')"
        c.execute(qry)
        c.execute(qr)
...
def Shop_Registration(request):
    if request.POST:
        ...
        regno = request.POST.get("regno")
        password = request.POST.get("Password")
        qry = "insert into shopreg(shop,district,city,phone,email,reg_no,password,status) values('" + cname + "','" + \
            district + "','" + location + "','" + mobile + "','" + \
            email + "','" + regno + "','" + password + "','Registered')"
...
def Admin_View_Feedback(request):
    data = ""
    c.execute(
        "select feedback.*,cusreg.* from feedback inner join cusreg on cusreg.cid=feedback.userid")
    data = c.fetchall()
...
def CustomerShopping(request):
    ...
    s = "select products.*,shopreg.shop,shopreg.email from products inner join categories on products.product_cat=categories.cat_id inner join shopreg on products.shid=shopreg.sid where categories.cat_title='" + \
        str(cat)+"' and (products.product_keywords like '%"+str(br) + \
        "%' or products.product_title like '%"+str(br)+"%')"
    c.execute(s)
    print(s)
    data = c.fetchall()
    print(data)
    ...
    return render(request, 'CustomerShopping.html', {"data": data, "data1": data1})
...
def CustomerViewProDetails(Request):
    ...
    data = c.fetchall()
    prodname = data[0][3]
    s = "select products.*,shopreg.shop,shopreg.email from products,shopreg where  products.shid=shopreg.sid and products.product_id <> '" + \
        str(pid)+"' and products.product_title='"+str(prodname)+"'"
    print(s)
    c.execute(s)
    compdata = c.fetchall()
    t = "select * from categories"
    ...
    if(Request.POST):
        ...
        c.execute("insert into cart (cid,pid,qty,price)values('" +
                  str(cid)+"','"+str(pid)+"','"+str(qty)+"','"+str(am)+"')")
        db.commit()
    return render(Request, 'CustomerViewProDetails.html', {"data": data, "data1": data1, "sna": sna, "em": em, "compdata": compdata})


def CustomerViewCart(request):
    cid = request.session['uid']
    s = "select * from cart inner join products on cart.pid = products.product_id where cart.cid = '" + \
        str(cid)+"'"
    c.execute(s)
    data = c.fetchall()
    # merid = data[0][9]
    ...
    if(request.POST):
        ...
        for d3 in data3:
            ...
            quty = d3[3]
            carid = d3[0]
            c.execute("insert into customer_order (uid,pid,p_price,p_qty)values('" +
                      str(cid)+"','"+str(proid)+"','"+str(amot)+"','"+str(quty)+"')")
            db.commit()
...
def Shop_Add_Product(request):
    ...
    if request.POST:
        ...
        if request.FILES.get("file"):
            ...
            c.execute("insert into products(product_cat,product_brand,product_title,product_price,quantity,product_desc,product_image,product_keywords,shid) values('" + str(
                b) + "','" + str(c1) + "','" + str(a) + "','" + str(d) + "','" + str(e) + "','" + str(f) + "','" + uploaded_file_url + "','" + str(g) + "','" + str(sid) + "')")
            db.commit()
...
def Shop_View_Orders(request):
    ...
    m = "SELECT customer_order.*, cusreg.*,products.* FROM customer_order INNER JOIN cusreg ON customer_order.uid=cusreg.cid inner join products on products.product_id=customer_order.pid where products.shid = '" + \
        str(shid)+"'"
    c.execute(m)
    data = c.fetchall()
    print(data)
...
def Shop_View_payment(request):
    shid = request.session['sid']
    c.execute("select cusreg.email,cusreg.cname,products.*,customer_order.* from products inner join customer_order on customer_order.pid=products.product_id inner join cusreg  on cusreg.cid=customer_order.uid where products.shid = '"+str(shid)+"'")
    data = c.fetchall()
    print(data)
...
def ShopUpdateProduct(request):
    ...
    if request.POST:
        ...
        qty = request.POST.get("qty")
        c.execute("update products set product_price = '"+str(price) +
                  "', quantity = '"+str(qty)+"' where product_id = '"+str(pid)+"'")
        db.commit()
        return HttpResponseRedirect("/ShopViewMyProduct/")
...
def Customer_View_Profile(request):
    ...
    if request.session['uid']:
        ...
        if request.POST:
            ...
            c.execute("update cusreg set mobile='"+Phn +
                      "' and address='"+Address+"' where cid='"+str(uid)+"'")
            db.commit()
...
def Customer_AddFeedback(request):
    ...
    if request.POST:
        ...
        m = "insert into feedback(feedback,date,userid) values('" + \
            str(descr) + "','" + str(da) + "','" + str(uid) + "')"
        c.execute(m)
        db.commit()
